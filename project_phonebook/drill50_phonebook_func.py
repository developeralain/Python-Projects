...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0) #this line clears all memory of any app widgets/objects in RAM to prevent
                    #memory leaks
                    #os is a module. _exit is a method. 0 is an argument we pass in.


#=========================================================
def create_db(self):
    conn = sqlite3.connect('db_phonebook.db')#if this isn't already there, it creates this dB
    with conn:
        cur = conn.cursor() #cursor method needed for SQL queries and statements
        ...
        conn.commit()
    conn.close()
    first_run(self)#this calls the first_run function which populates the dB with values
                    #the first time its run to prevent errors 
...
def onSelect(self,event):
    ...
    with conn:
        ...
        varBody = cursor.fetchall()#fetches data outside sql dB
